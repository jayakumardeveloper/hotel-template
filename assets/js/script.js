...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Bind Search button click helper
  const searchBtn = document.getElementById('search-btn');
  if (searchBtn) {
    searchBtn.addEventListener('click', (e) => {
      e.preventDefault();
      showToast('Search feature: Please use the category filters below to explore our menus!', '#ffbe0e');
    });
  }
          ...
          const cardHTML = `
            <div class="menu-card" data-category="${item.category}">
              <div class="card-img-container">
                ...
                <img src="${item.image}" alt="${item.alt}" class="card-img" loading="lazy" />
              </div>
  ...
  if (locationsMapEl && typeof L !== 'undefined') {
    const branches = [
      { lat: 11.9215459, lng: 79.8268495, name: 'Hotel Sidambaram', mapsUrl: 'https://maps.app.goo.gl/czDDMJssp7rnr7Lx5' },
      { lat: 11.9296506, lng: 79.8282011, name: 'Hotel Sidambaram', mapsUrl: 'https://maps.app.goo.gl/Yhc3BzVS1DQyLz5y8' },
      { lat: 11.9297944, lng: 79.8282073, name: 'Hotel Sidambaram', mapsUrl: 'https://maps.app.goo.gl/VMTFc4DeYA9ZmmaT8' },
      { lat: 11.9351495, lng: 79.800506, name: 'Hotel Sidambaram', mapsUrl: 'https://maps.app.goo.gl/Ghy1k5iLyyyU5yWw6' },
    ];
    ...
    branches.forEach((branch, idx) => {
      ...
      marker.bindPopup(`<strong>${branch.name} - Branch ${idx + 1}</strong><br/><a href="${branch.mapsUrl}" target="_blank" rel="noopener">Get Directions</a>`);
  ...
  if (orderItemsList) {
    ...
    function renderOrderPage() {
      ...
      if (orderCouponMsg && discountPercent > 0) {
        if (cart.length === 0) {
          ...
          if (appliedPct > 0) {
            orderCouponMsg.textContent = `Coupon "${code}" applied! ${appliedPct}% off.`;
            orderCouponMsg.className = 'order-coupon-msg success';
          } else {
            orderCouponMsg.textContent = `Coupon "${code}" applied! (Discount starts from ₹100)`;
            orderCouponMsg.className = 'order-coupon-msg success';
          }
        }
      }
    ...
    if (addressPickerMapEl && typeof L !== 'undefined') {
